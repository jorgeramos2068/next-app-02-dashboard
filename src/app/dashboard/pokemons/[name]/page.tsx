...
import { getPokemon } from '@/helpers/functions';
...
export async function generateStaticParams() {
  const pokemonResponse: PokemonsResponse = await fetch('https://pokeapi.co/api/v2/pokemon?limit=151').then(res =>
    res.json()
  );
  const staticList = pokemonResponse.results.map(pokemon => ({
    name: pokemon.name,
  }));
  return staticList.map(({ name }) => ({ name }));
}
